...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        public DbContext(string connetionString)
        {
            this.connection = new DatabaseConnection(connetionString);
        ...
        public void SaveChanges()
        {
            ...
            {
                var invalidEntities = this.dbSetProperties.Where(entity => !IsObjectValid(entity)).ToArray();
            ...
            where TEntity : class, new()
        {
            ...
            var columns = this.connection.FetchColumnNames(tableName.ToString()).ToArray();

            if (dbSet.ChangeTracker.Added.Any())
            {
                this.connection.InsertEntities(dbSet.ChangeTracker.Added, tableName.ToString(), columns);
            }
            ...
            {
                this.connection.UpdateEntities(modifiedEntities, tableName.ToString(), columns);
            }

            if (dbSet.ChangeTracker.Removed.Any())
            {
                this.connection.DeleteEntities(dbSet.ChangeTracker.Removed, tableName.ToString(), columns);
            }
        }
        ...
        private void PopulateDbSet<TEntity>(PropertyInfo dbSet)
           where TEntity : class, new()
        {
            var tablEntitiesentities = LoadTableEntities<TEntity>();
            var dbSetInstance = new DbSet<TEntity>(tablEntitiesentities);
            ...
            where TEntity : class
        {
            ...
            var columns = GetColumnsNames(table);
