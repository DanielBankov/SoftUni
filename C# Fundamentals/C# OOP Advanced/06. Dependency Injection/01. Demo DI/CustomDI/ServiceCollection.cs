...
namespace CustomDI
{
    public class ServiceCollection : IServiceCollection
    {
        ...
        public object CreateInstance(Type type)
        {
            ...
            var constructorParameterObject = new List<object>();

            foreach (var constructorParameter in constructorParameters)
            {
                var parameterObject = this.CreateInstance(constructorParameter.ParameterType);
                constructorParameterObject.Add(parameterObject);
            }

            var obj = constructor.Invoke(constructorParameterObject.ToArray());
            return obj;
        }
    }
}
