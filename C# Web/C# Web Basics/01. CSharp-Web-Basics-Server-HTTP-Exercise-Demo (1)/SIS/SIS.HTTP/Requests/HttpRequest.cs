...
namespace SIS.HTTP.Requests
{
    public class HttpRequest : IHttpRequest
    {
        ...
        {
            if (this.Headers.ContainsHeader(GlobalConstants.HeaderCookie))
            {
                string value = this.Headers.GetHeader(GlobalConstants.HeaderCookie).Value;

                //Cookie: language=bg; ASP.NET_SessionId=vttkvicad10joulgdnafe5hh;
                var unparsedCookies = value.Split(new[] { "; " }, StringSplitOptions.RemoveEmptyEntries);

                foreach (var unparsedCookie in unparsedCookies)
                {
                    string[] cookieKeyValuePair = unparsedCookie.Split(new[] { '=' }, 2).ToArray();
        ...
        private void ParseRequest(string requestString)
        {
            ...
            this.ParseRequestParameters(splitRequestString[splitRequestString.Length - 1]);//body
        }
    }
}
